...
describe('EventBatchEditDlg.vue', () => {
    ...
    it.each([EventState.Draft, EventState.OpenForSignup, EventState.Planned, EventState.Canceled])(
        ...
        async (state) => {
            const input = testee.find('[data-test-id="input-event-status"] input');
    ...
    it('should patch only event name', async () => {
        const input = testee.find('[data-test-id="input-event-name"] input');
    ...
    it.each([
        ...
        const input = testee.find('[data-test-id="input-event-type"] input');
        await input.trigger('click');
    ...
    it.each([EventSignupType.Assignment, EventSignupType.Open])('should patch only event signup type to $0', async (signupType) => {
        const input = testee.find('[data-test-id="input-event-signup-type"] input');
        await input.trigger('click');
    ...
    it('should patch only event slots', async () => {
        const input = testee.find('[data-test-id="input-event-slots"] input');
        await input.trigger('click');
    ...
    it('should patch only event description', async () => {
        const input = testee.find('[data-test-id="input-event-description"] textarea');
        await input.setValue('updated');
        await submit();
    ...
    it('should patch event name and description', async () => {
        const name = testee.find('[data-test-id="input-event-name"] input');
        await name.setValue('updated');
        const description = testee.find('[data-test-id="input-event-description"] textarea');
