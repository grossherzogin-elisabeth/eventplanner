<template>
    ...
    <!-- crew -->
    <td class="hidden w-1/6 min-w-16 whitespace-nowrap md:table-cell" :class="{ 'opacity-50': isPastEvent }">
        <template v-if="showWaitingList">
    ...
    <td class="hidden w-1/6 md:table-cell" :class="{ 'opacity-50': isPastEvent }">
        <div class="flex items-center lg:justify-end">
            <div :key="stateDetails.icon" class="status-badge" :class="stateDetails.color">
                <i class="fa-solid w-4" :class="stateDetails.icon"></i>
                <span>{{ stateDetails.name }}</span>
            </div>
        </div>
