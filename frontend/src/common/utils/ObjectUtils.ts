 ...
 */
export function hash<T>(object: T): string {
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function extractValue(item: any, field: string): any {
    ...
    if (Array.isArray(value)) {
        ...
        console.warn(`Nested array indices are currently not supported in table sorting! Tried to extract ${next}.`);
        return undefined;
    }
    if (typeof value === 'object') {
        if (parts.length === 0) {
        ...
        return extractValue(value, parts.join('.'));
    }
...
export function diff<T extends object>(oldValue: T, newValue: T): Partial<T> {
...
export function deepEquals(a: unknown, b: unknown): boolean {
    ...
    if (typeof a === 'object' || typeof b === 'object') {
        ...
        const keys = Object.keys(a).concat(Object.keys(b)).filter(filterDuplicates);
        for (const key of keys) {
            const a2 = (a as Record<string, unknown>)[key];
            const b2 = (b as Record<string, unknown>)[key];
            if (!deepEquals(a2, b2)) return false;
        }
        return true;
    }
    return a === b;
