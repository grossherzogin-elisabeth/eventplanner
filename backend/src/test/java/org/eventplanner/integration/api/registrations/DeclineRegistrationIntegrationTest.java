...
class DeclineRegistrationIntegrationTest extends EmailSpy {
    ...
    void shouldRequireAuthentication() throws Exception {
        webMvc.perform(put("/api/v1/events/" + randomUUID() + "/registrations/" + randomUUID() + "/decline")
    ...
    void shouldRequireAuthorization() throws Exception {
        ...
        webMvc.perform(put("/api/v1/events/" + event.getKey() + "/registrations/" + registration.getKey() + "/decline")
    ...
    void shouldReturnNotFoundForUnknownRegistration() throws Exception {
        ...
        webMvc.perform(put("/api/v1/events/" + event.getKey() + "/registrations/" + randomUUID() + "/decline")
                .with(withAuthentication(TestUser.TEAM_MEMBER))
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isNotFound());
    ...
    @Test
    void shouldDeclineRegistration() throws Exception {
        ...
        assertThat(registrationJpaRepository.findById(registration.getKey().value())).isEmpty();
    }

    @Test
    void shouldDeclineRegistrationWithAccessKeyAuthentication() throws Exception {
        var registration = createRegistration().withUserKey(TestUser.TEAM_MEMBER.getKey());
        registration.setConfirmedAt(null);
        var event = createEvent().withRegistrations(List.of(registration, createRegistration(), createRegistration()));
        saveTestEvent(event);
    ...
    @Test
    void shouldNotDeclineRegistrationOfAnotherUser() throws Exception {
        ...
        assertThat(registrationJpaRepository.findById(registration.getKey().value())).isNotEmpty();
    }
